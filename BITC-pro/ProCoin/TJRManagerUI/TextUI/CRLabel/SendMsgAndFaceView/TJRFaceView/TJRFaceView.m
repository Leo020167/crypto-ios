...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
	if ([@"frame" isEqualToString:keyPath]) {
		...
		if (phoneWidth > self.frame.size.width) {
...
- (void)initializationFaceView {
    [[NSBundle mainBundle] loadNibNamed:@"TJRFaceView" owner:self options:nil];
...
- (void)interfaceChange {
	...
	//[self performSelectorInBackground:@selector(creactFaceView) withObject:nil];
    [self creactFaceView];
...
//    if ([faceName integerValue] == BackspaceTag) {
//        index = MaxRank * MaxRow - 1;
//    }
//	float pointY = index / MaxRank % MaxRow * (FaceHeight + VerticalSpacing) + VerticalSpacing;
//	float pointX = (index % MaxRank) * (FaceWidth + HorizontalSpacing) + HorizontalSpacing + (currentPage * self.frame.size.width);
...
//    btnFace.frame = CGRectMake(pointX, pointY, FaceWidth, FaceHeight);
