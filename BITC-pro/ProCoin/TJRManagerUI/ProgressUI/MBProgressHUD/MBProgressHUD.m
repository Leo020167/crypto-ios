...
@property (MB_STRONG) NSTimer *timeOutTimer;
...
- (void)show:(BOOL)animated {
    ...
    self.bounds = self.backgroundRect;
    //[self backgroundRectAuto];
    ...
    self.timeOutTimer=[NSTimer scheduledTimerWithTimeInterval:timeOutMax target:self selector:@selector(hide:) userInfo:nil repeats:NO];
    
    if (mode == MBProgressHUDModeTJRView) {
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		[self addSubview:indicator];
    }else if (mode == MBProgressHUDModeTJRView) {
        
        if (customView == nil) {
            customView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 50, 50)];
        }
        CAShapeLayer *shapeLayer = [[[CAShapeLayer alloc] init]autorelease];
        shapeLayer.fillColor = nil;
        shapeLayer.strokeColor = [UIColor colorWithRed:128.0/255.0 green:128.0/255.0 blue:128.0/255.0 alpha:1].CGColor;
...
- (void)layoutSubviews {
    ...
    CGSize labelSize = [label.text boundingRectWithSize:constraint options:(NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin) attributes:@{NSFontAttributeName:label.font} context:nil].size;
    ...
    CGSize detailsConstraint = CGSizeMake(maxSize.width, 20000.0f);
    CGSize detailsLabelSize = [detailsLabel.text boundingRectWithSize:detailsConstraint options:(NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin) attributes:@{NSFontAttributeName:detailsLabel.font} context:nil].size;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetRGBFillColor(context, 81.0/255.0, 81.0/255.0, 81.0/255.0, self.opacity);
...
- (void)deviceOrientationDidChange:(NSNotification *)notification { 
	...
	if (!superview) {
        ...
        if (UIInterfaceOrientationIsLandscape(orientation)) {
            ...
            self.bounds = CGRectMake(0, 44 + statusBarHeight,[[UIScreen mainScreen] bounds].size.height, [[UIScreen mainScreen] bounds].size.width - 44*2 - statusBarHeight);
        } else {
            self.bounds = self.backgroundRect;
        }
...
- (void)startIndeterminateAnimation
{
    ...
    self.shapeLayer.frame = indicator.bounds;
    ...
    if (!animation) {
        ...
        for (int i = 0; i < 33; i++) {
            ...
            [values insertObject:(id)[UIImage imageNamed:[NSString stringWithFormat:@"white_loading_icon_%@",num]].CGImage atIndex:i];
        }
