...
- (void)show:(UIView *)parent {
	...
	CGSize textSize = [CommonUtil getPerfectSizeByText:text andFontSize:font.pointSize andWidth:280];
//	CGSize textSize = [text sizeWithFont:font constrainedToSize:CGSizeMake(280, 60)];
	...
	CGPoint point = CGPointMake(parent.bounds.size.width / 2, parent.bounds.size.height / 2);

	if (theSettings.gravity == iToastGravityTop) {
		point = CGPointMake(parent.bounds.size.width / 2, 65);
	} else if (theSettings.gravity == iToastGravityBottom) {
		point = CGPointMake(parent.bounds.size.width / 2, parent.bounds.size.height - 90);
	} else if (theSettings.gravity == iToastGravityCenter) {
		point = CGPointMake(parent.bounds.size.width / 2, parent.bounds.size.height / 2 - 60);
	} else {
