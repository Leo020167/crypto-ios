...
- (void)P_btn_action:(UIButton *)button {
    ...
    // 点击选中按钮时
    if (self.signBtnIndex == button.tag && [_delegatePageTitleView respondsToSelector:@selector(pageTitleView:didClickedButton:)]) {
        [self.delegatePageTitleView pageTitleView:self didClickedButton:button];
    }
...
// 获取当前高亮按钮的下标
- (NSInteger)getCurrentSignBtnIndex {
    return _signBtnIndex;
}
