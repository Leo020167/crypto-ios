...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        // 解决_collectionView跟左边缘返回手势的冲突
        [self setScreenEdgePanGestureRecognizerPriority:YES];
...
// 获取子控制器
- (NSArray *)getchildViewControllerArray {
    return  _childViewControllers;
}

/**  设置UINavigationController的左边缘滑动返回手势优先级高于scrollercView的滑动手势 */
- (void)setScreenEdgePanGestureRecognizerPriority :(BOOL)isCanDragBack{
    ...
    UIScreenEdgePanGestureRecognizer *leftGR = (UIScreenEdgePanGestureRecognizer *)navigation.interactivePopGestureRecognizer;
    if (isCanDragBack) {
        // 当返回手势失效时才执行滑动手势
        [self.collectionView.panGestureRecognizer requireGestureRecognizerToFail:leftGR];
    }else {
        // 当滑动手势失效时才执行返回手势
        [leftGR requireGestureRecognizerToFail:self.collectionView.panGestureRecognizer];
    }
