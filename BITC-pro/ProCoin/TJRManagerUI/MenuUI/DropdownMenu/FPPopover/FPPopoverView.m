...
#define FP_POPOVER_TITLEHEIGHT 20.0
...
-(CGPathRef)createContentPathWithBorderWidth:(CGFloat)borderWidth arrowDirection:(FPPopoverArrowDirection)direction
{
    ...
    CGFloat h = self.bounds.size.height - 27;
...
-(CGGradientRef)createGradient
{
    ...
    else if(self.tint == FPPopoverRedTint)
    {
        if(_arrowDirection == FPPopoverArrowDirectionUp)
        {
            colors[0] = colors[4] = 0.36;
            colors[1] = colors[5] = 0.00;
            colors[2] = colors[6] = 0.09;
            colors[3] = colors[7] = 0.95;
    ...
    else if(self.tint == FPPopoverGreenTint)
    {
        if(_arrowDirection == FPPopoverArrowDirectionUp)
        {
            colors[0] = colors[4] = 0.18;
            colors[1] = colors[5] = 0.30;
            colors[2] = colors[6] = 0.03;
            colors[3] = colors[7] = 0.95;
    ...
    if(self.tint == FPPopoverBlueTint)
    {
        if(_arrowDirection == FPPopoverArrowDirectionUp)
        {
            colors[0] = 0;
            colors[1] = 113.0/255.0;
            colors[2] = 188.0/255.0;
            colors[4] = 0;
            colors[5] = 113.0/255.0;
            colors[6] = 188.0/255.0;
            colors[3] = colors[7] = 0.95;
        }
        else
        {
...
- (void)drawRect:(CGRect)rect
{
    ...
    {
        CGContextSetRGBFillColor(ctx, 0 , 113.0/255.0, 188.0/255.0, 0.95);        
    }
...
-(void)setupViews
{
    ...
    if(_arrowDirection == FPPopoverArrowDirectionUp)
    {
        contentRect.origin = CGPointMake(10, FP_POPOVER_TITLEHEIGHT);  
