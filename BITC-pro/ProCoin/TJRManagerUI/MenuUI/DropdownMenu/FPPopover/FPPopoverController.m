...
@synthesize localSize= _localSize;
-(void)addObservers
{
...
-(id)initWithViewController:(UIViewController*)viewController
{
    ...
    if(self)
    {
        ...
        self.contentSize = CGSizeMake(200, 295); //default size

        _contentView = [[FPPopoverView alloc] initWithFrame:CGRectMake(0, 0,
                                              self.contentSize.height, self.contentSize.width)];
        
        _viewController = viewController;//传入参数，不释放
...
-(void)setupView
{
    if (UIInterfaceOrientationIsPortrait(_deviceOrientation)) {
         self.view.frame = CGRectMake(0, 0, [self parentWidth], [self parentHeight]);
    }else{
        self.view.frame = CGRectMake(0, 0, [self parentHeight], [self parentWidth]);
    }
...
{
    if (UIInterfaceOrientationIsPortrait(_deviceOrientation)) {
        if(_parentView.bounds.size.width>[[UIScreen mainScreen] bounds].size.width)
            return _localSize.width;
        return _parentView.bounds.size.width;
    }else{
        return _parentView.bounds.size.height;
    }
    return _parentView.bounds.size.width;
    return UIInterfaceOrientationIsPortrait(_deviceOrientation) ? _parentView.frame.size.width : _parentView.frame.size.height;
}
-(CGFloat)parentHeight
{
    ...
    return _parentView.bounds.size.height;
    return UIInterfaceOrientationIsPortrait(_deviceOrientation) ? _parentView.frame.size.height : _parentView.frame.size.width;
}
...
-(CGRect)bestArrowDirectionAndFrameFromView:(UIView*)v
{
    ...
    if (r.size.width>[[UIScreen mainScreen] bounds].size.width) {
        r.size.width=_localSize.width;
    }
    ...
    r.size.width=r.size.width>[[UIScreen mainScreen] bounds].size.width?_localSize.width:r.size.width;
    _contentView.arrowDirection = bestDirection;
    
    if (bCustomSetting) {
        _contentView.frame = CGRectMake(_contentView.frame.origin.x, _contentView.frame.origin.y, r.size.width, r.size.height);
    }else{
    ...
    if (_fromView) {
        ...
        id next = _fromView.nextResponder;
        int max = 10;
        while(1)
            //如果nextResponder没有当前这个view放入ViewController中会造成死循环，故设置最大数为10。
        {
            if ([next isKindOfClass:[UIViewController class]] || max <=0 || next == nil) {
                break;
            }
        ...
        if ([next isKindOfClass:[UIViewController class]]) {
            //生成在UIViewController之上
            controller =(UIViewController*)next;
        }
    }
    id ctr =controller.navigationController.view;
    if (ctr) {
        _contentView.relativeOrigin = [ctr convertPoint:self.origin toView:_contentView];
    }
    else
    {
        _contentView.relativeOrigin = [_parentView convertPoint:self.origin toView:_contentView];
...
/**
 *	@brief	set ContentView's Origin by yourself
 *
 *	@param 	origin 	origin point
 */
-(void)setContentViewOrigin:(CGPoint)origin
{
    bCustomSetting = YES;
    _contentView.frame= CGRectMake(origin.x, origin.y, _contentView.frame.size.width, _contentView.frame.size.height);
}

-(void)setIsBorder:(BOOL)border
{
    _contentView.isBorder=border;
    [_contentView setNeedsDisplay];
}
