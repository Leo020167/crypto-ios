...
int ReadPCMFrame(short speech[], FILE *fpwave, int nChannels, int nBitsPerSample) {
	...
	if ((nBitsPerSample == 8) && (nChannels == 1)) {
		nRead = (unsigned int)fread(pcmFrame_8b1, (nBitsPerSample / 8), PCM_FRAME_SIZE * nChannels, fpwave);

		for (x = 0; x < PCM_FRAME_SIZE; x++) {
			speech[x] = (short)((short)pcmFrame_8b1[x] << 7);
		}
	} else if ((nBitsPerSample == 8) && (nChannels == 2)) {
		nRead = (unsigned int)fread(pcmFrame_8b2, (nBitsPerSample / 8), PCM_FRAME_SIZE * nChannels, fpwave);
		...
		nRead = (unsigned int)fread(pcmFrame_16b1, (nBitsPerSample / 8), PCM_FRAME_SIZE * nChannels, fpwave);

		for (x = 0; x < PCM_FRAME_SIZE; x++) {
			speech[x] = (short)pcmFrame_16b1[x + 0];
		}
	} else if ((nBitsPerSample == 16) && (nChannels == 2)) {
		nRead = (unsigned int)fread(pcmFrame_16b2, (nBitsPerSample / 8), PCM_FRAME_SIZE * nChannels, fpwave);
...
int EncodeWAVEFileToAMRFile(const char *pchWAVEFilename, const char *pchAMRFileName, int nChannels, int nBitsPerSample) {
	...
	bytes = (unsigned int)fwrite(AMR_MAGIC_NUMBER, sizeof(char), strlen(AMR_MAGIC_NUMBER), fpamr);
...
#pragma clang diagnostic pop
	...
	while (1) {
		bytes = (unsigned int)fread(&frameHeader, 1, sizeof(unsigned char), fpamr);
	...
	bytes = (unsigned int)fread(&(frameBuffer[1]), 1, (stdFrameSize - 1) * sizeof(unsigned char), fpamr);
