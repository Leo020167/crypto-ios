...
-(id)initWithFrame:(CGRect)frame title:(NSString *)title description:(NSString *)description image:(UIImage *)image{
    ...
    if (self) {
        ...
        self.PanelTitle = title;
        self.PanelDescription = description;
        self.PanelImageView = [[[UIImageView alloc] initWithImage:image] autorelease];
        [self buildPanelWithFrame:frame];
    }
    return self;
}
...
-(id)initWithFrame:(CGRect)frame title:(NSString *)title description:(NSString *)description footer:(UIView *)footerView header:(UIView *)headerView{
    self = [super initWithFrame:frame];
    if (self) {
        // Initialization code
        [self initializeConstants];
        self.PanelFooterView = footerView;
        self.PanelHeaderView = headerView;
        self.PanelTitle = title;
        self.PanelDescription = description;
        [self buildPanelWithFrame:frame];
...
-(void)buildPanelWithFrame:(CGRect)frame{
    ...
    //Create title label
    self.PanelTitleLabel = [[[UILabel alloc] initWithFrame:CGRectMake(kLeftRightMargins, runningYOffset, frame.size.width - 2*kLeftRightMargins, panelTitleHeight)] autorelease];
    ...
    self.PanelSeparatorLine = [[[UIView alloc] initWithFrame:CGRectMake(kLeftRightMargins, runningYOffset - 0.5*kTitleDescriptionPadding, frame.size.width - 2*kLeftRightMargins, 1)] autorelease];
    self.PanelSeparatorLine.backgroundColor = kSeparatorLineColor;
     [self addSubview:self.PanelSeparatorLine];
    
    //Create description label
    self.PanelDescriptionLabel = [[[UILabel alloc] initWithFrame:CGRectMake(kLeftRightMargins, runningYOffset, frame.size.width - 2*kLeftRightMargins, panelDescriptionHeight)] autorelease];
    ...
    CGFloat padding = 0;
    if (IS_IPHONE_6P || IS_IPHONE_6P_OR_MORE) {
        padding = 50;
    }else if (IS_IPHONE_5_OR_MORE) {
        padding = 20;
    }
    
    runningYOffset += panelDescriptionHeight + padding + kDescriptionImagePadding;
    
    
    if (self.PanelFooterView) {
        self.PanelFooterView.frame = CGRectMake(0, runningYOffset, self.frame.size.width, self.frame.size.height);
    }else{
        self.PanelFooterView = [[[UIView alloc]initWithFrame:CGRectMake(0, runningYOffset, self.frame.size.width, self.frame.size.height)]autorelease];
    }
    self.PanelFooterView.clipsToBounds = YES;
    self.PanelFooterView.backgroundColor = [UIColor clearColor];
    [self addSubview:self.PanelFooterView];
    
    //Add image, if there is room
    if (self.PanelImageView.image) {
        self.PanelImageView.frame = CGRectMake(0, 0, self.PanelFooterView.frame.size.width, self.PanelFooterView.frame.size.height);
        self.PanelImageView.contentMode = UIViewContentModeScaleAspectFit;
        self.PanelImageView.clipsToBounds = YES;
        [self.PanelFooterView addSubview:self.PanelImageView];
    }
...
//- (void)setkImagePadding:(CGFloat)padding{
//    kDescriptionImagePadding = padding;
//}
...
-(void)panelDidAppear{
    ...
    if ([self.PanelFooterView isKindOfClass:[MYIntroductionPanel class]]) {
        [((MYIntroductionPanel*)self.PanelFooterView) panelDidAppear];
    }
    
    if ([self.PanelHeaderView isKindOfClass:[MYIntroductionPanel class]]) {
        [((MYIntroductionPanel*)self.PanelHeaderView) panelDidAppear];
    }
}
...
-(void)panelDidDisappear{
    //Implemented by subclass
    if ([self.PanelFooterView isKindOfClass:[MYIntroductionPanel class]]) {
        [((MYIntroductionPanel*)self.PanelFooterView) panelDidDisappear];
    }
    
    if ([self.PanelHeaderView isKindOfClass:[MYIntroductionPanel class]]) {
        [((MYIntroductionPanel*)self.PanelHeaderView) panelDidDisappear];
    }
}

- (void)dealloc{

    [_PanelHeaderView release];
    [_PanelTitle release];
    [_PanelDescription release];
    [_PanelTitleLabel release];
    [_PanelDescriptionLabel release];
    [_PanelSeparatorLine release];
    [_PanelImageView release];
    [_PanelFooterView release];
    [super dealloc];
}
@end
