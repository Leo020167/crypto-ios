...
-(void)initializeViewComponents{
   ...
   //[self setAutoresizingMask: UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth];
    ...
    //Master Scroll View
    self.MasterScrollView = [[[UIScrollView alloc] initWithFrame:self.frame] autorelease];
    self.MasterScrollView.autoresizingMask =  UIViewAutoresizingFlexibleHeight;
    ...
    //Page Control
    self.PageControl = [[[UIPageControl alloc] initWithFrame:CGRectMake((self.frame.size.width - kPageControlWidth)/2, self.frame.size.height - 77, kPageControlWidth, 37)] autorelease];
    //[self.PageControl setAutoresizingMask: UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleLeftMargin];
    ...
    if ([MYIntroductionPanel runningiOS7]) {
        ...
        skipStringWidth = [skipString boundingRectWithSize:constraint options:(NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin) attributes:@{NSFontAttributeName:kSkipButtonFont} context:nil].size.width;
    }
    ...
    [self.LeftSkipButton setAutoresizingMask: UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleLeftMargin];
    ...
    [self.RightSkipButton setAutoresizingMask: UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleLeftMargin];
...
-(void)animatePanelAtIndex:(NSInteger)index{
    ...
    for (MYIntroductionPanel *panelView in Panels) {
        ...
        panelView.PanelFooterView.alpha = 0;
    }
    ...
    if (Panels.count > index) {
        ...
        CGRect initialImageFrame = [Panels[index] PanelFooterView].frame;
        ...
        [[Panels[index] PanelFooterView] setFrame:CGRectMake(initialImageFrame.origin.x, initialImageFrame.origin.y + 10, initialImageFrame.size.width, initialImageFrame.size.height)];
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            [UIView animateWithDuration:0.3 animations:^{
                ...
                [[Panels[index] PanelFooterView] setFrame:initialImageFrame];
            }];
        }];
    }
}
...
-(void)changeToPanelAtIndex:(NSInteger)index{
    self.LanguageDirection = MYLanguageDirectionLeftToRight;
    self.CurrentPanelIndex = index -1;
    [self.MasterScrollView setContentOffset:CGPointMake(index * self.frame.size.width, 0) animated:YES];
...
- (void)dealloc{
    ...
    [_MasterScrollView release];
    [_PageControl release];
    [_RightSkipButton release];
    [_LeftSkipButton release];
    [super dealloc];
}
