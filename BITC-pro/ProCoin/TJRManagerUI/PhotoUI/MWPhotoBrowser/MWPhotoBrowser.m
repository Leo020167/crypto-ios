...
@interface MWPhotoBrowser () {
	...
	UIBarButtonItem  *_actionButton, *_backButton, *_titleBar;
...
@synthesize displayActionButton = _displayActionButton, actionsSheet = _actionsSheet,displayEditButton =_displayEditButton;
...
#pragma clang diagnostic pop
    ...
    if ([[UIToolbar class] respondsToSelector:@selector(appearance)]) {
        //[_toolbar setBackgroundImage:[UIImage imageNamed:@"light_gray_nav_bar.png"] forToolbarPosition:UIToolbarPositionAny barMetrics:UIBarMetricsDefault];
    ...
    [_toolbar setBackgroundImage:transparentImage forToolbarPosition:UIToolbarPositionAny barMetrics:UIBarMetricsDefault];
    
    _toolbar.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleWidth;

    
    _titleBar = [[UIBarButtonItem alloc] initWithTitle:@"" style:UIBarButtonItemStylePlain target:self action:nil];
    
    _actionButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"quotation_more_action.png"] style:UIBarButtonItemStylePlain target:self action:@selector(actionButtonPressed:)];
    
    _backButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"tjrpublic_btn_back.png"] style:UIBarButtonItemStylePlain target:self action:@selector(backButtonPressed:)];
...
- (void)performLayout {
    ...
    if (_backButton) [items addObject:_backButton];
    if (_displayActionButton) [items addObject:fixedLeftSpace];
    [items addObject:flexSpace];
    if (_titleBar) [items addObject:_titleBar];
    [items addObject:flexSpace];
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        UIBarButtonItem *doneButton = [[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"完成", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)] autorelease];
        // Set appearance
...
- (void)reloadData {
    if (CGRectEqualToRect(CGRectZero, showFrame)) {
...
- (CGPoint)contentOffsetForPageAtIndex:(NSUInteger)index {
    ...
    CGFloat y = 0;
    y = _isHideToolBar ? 0 : y;
	return CGPointMake(newOffset, y);
}

- (CGRect)frameForToolbarAtOrientation:(UIInterfaceOrientation)orientation {
//    CGFloat height = CURRENT_DEVICE_VERSION>=7.0?(48+20):48;
//    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone &&
//        UIInterfaceOrientationIsLandscape(orientation)) height = CURRENT_DEVICE_VERSION>=7.0?(32+20):32;
//    
//	return CGRectMake(0, CURRENT_DEVICE_VERSION>=7.0?0:20, self.view.bounds.size.width, height);
    CGFloat height = 48;
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone &&
...
- (CGRect)frameForCaptionView:(MWCaptionView *)captionView atIndex:(NSUInteger)index {
    ...
    CGRect captionFrame = CGRectMake(pageFrame.origin.x, pageFrame.size.height - captionSize.height - 0, pageFrame.size.width, captionSize.height);
    return captionFrame;
}
...
- (void)updateNavigation {
	...
	if ([self numberOfPhotos] > 1) {
		_titleBar.title = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)_currentPageIndex+1, NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)[self numberOfPhotos]];
	} else {
		_titleBar.title = nil;
	}
}
...
- (void)backButtonPressed:(id)sender {
    ...
    if ([_delegate respondsToSelector:@selector(backButtonPressed:)]) {
        [_delegate backButtonPressed:self];
    }
}
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if (_displayEditButton) {
                _actionsSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        ...
                                                        otherButtonTitles:NSLocalizedString(@"保存到手机", nil), NSLocalizedString(@"复制到剪切板", nil),NSLocalizedString(@"编辑图片", nil), nil];
            }else{
                _actionsSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        cancelButtonTitle:NSLocalizedString(NSLocalizedStringForKey(@"取消"), nil) destructiveButtonTitle:nil
                                                        otherButtonTitles:NSLocalizedString(@"保存到手机", nil), NSLocalizedString(@"复制到剪切板", nil), nil];
            }
            // Sheet
...
- (void)editPhoto
{
    ...
    if ([_delegate respondsToSelector:@selector(editButtonPressed:)]) {
        [_delegate editButtonPressed:self];
    }
}
...
- (void)setIsHideToolBar:(BOOL)isHideToolBar {
    ...
    if (isHideToolBar) {
        _toolbar.hidden = YES;
        UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(closeBrowser:)];
        [self.view addGestureRecognizer:tapGesture];
        RELEASE(tapGesture);
        UILongPressGestureRecognizer *longGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(actionBrowser:)];
        [self.view addGestureRecognizer:longGesture];
        RELEASE(longGesture);
    }
}

- (void)closeBrowser:(UITapGestureRecognizer *)gesture {
    if (gesture.state == UIGestureRecognizerStateEnded) {
        [self popViewControllerWithNoBackButton];
    }
}

- (void)actionBrowser:(UILongPressGestureRecognizer *)gesture {
...
//    [self.view.superview.layer addAnimation:myTranstiton forKey:nil ];
    [((TJRAppDelegate *)[[UIApplication sharedApplication] delegate]).navigation pushViewController:self animated:NO];
...
//    }];
    ...
    self.view.alpha = 0.3;
    dismissFrame = disFrame;
    self.view.frame = dismissFrame;
...
- (void)popViewControllerWithNoBackButton {
    [((TJRAppDelegate *)[[UIApplication sharedApplication] delegate]).navigation popViewControllerAnimated:NO];
    if (_delegate && [_delegate respondsToSelector:@selector(didPopViewControllerWithImage:index:)]) {
       id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
        [_delegate didPopViewControllerWithImage:[photo underlyingImage] index:_currentPageIndex];
    }
}
