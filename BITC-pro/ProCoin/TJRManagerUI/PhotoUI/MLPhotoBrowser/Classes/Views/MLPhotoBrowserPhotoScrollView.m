...
- (void)longGesture:(UILongPressGestureRecognizer *)gesture{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (!self.sheet) {
            self.sheet = [[UIActionSheet alloc] initWithTitle:NSLocalizedStringForKey(@"提示") delegate:self cancelButtonTitle:NSLocalizedStringForKey(@"取消") destructiveButtonTitle:@"保存到相册" otherButtonTitles:nil, nil];
        }
    ...
    if (buttonIndex == 0){
        if([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeSavedPhotosAlbum]) {
            ...
            if (_photoImageView.image) {
                [self showMessageWithText:NSLocalizedStringForKey(@"保存成功")];
            }
        }else{
            if (_photoImageView.image) {
...
- (void)setPhoto:(MLPhotoBrowserPhoto *)photo{
    ...
    if (photo.photoURL.absoluteString.length) {
        // 本地
        UIImage *cachedImage = [UIImage imageWithData:[TTCacheManager dataForURL:photo.photoURL.absoluteString]];
        
        if (cachedImage){
            weakSelf.isLoadingDone = YES;
            weakSelf.photoImageView.image = cachedImage;
            ...
            [self loadImageWithURL:photo.photoURL.absoluteString];
        }
...
- (void)loadImageWithURL:(NSString *)imageUrl {
    ...
    if (nil != imageUrl) {
        ...
        if (nil != data) {
            ...
            if (imageUrl) {
                self.photoImageView.image = [UIImage imageNamed:imgDefault];
            }
        }
    }
}
