...
- (UIButton *)deleleBtn{
    if (!_deleleBtn) {
        ...
        //        [deleleBtn setTitle:NSLocalizedStringForKey(@"删除") forState:UIControlStateNormal];
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.photos.count) {
        ...
        MLPhotoBrowserPhoto *photo = self.photos[indexPath.item];
        ...
        MLPhotoBrowserPhotoScrollView* bPScrollView =  [[MLPhotoBrowserPhotoScrollView alloc] init];
        bPScrollView.tag = ScrollViewTAG;
        bPScrollView.sheet = self.sheet;
        bPScrollView.backgroundColor = [UIColor clearColor];
        // 为了监听单击photoView事件
        bPScrollView.frame = [UIScreen mainScreen].bounds;
        bPScrollView.photoScrollViewDelegate = self;
        if (bDidLoad) {
            bDidLoad = NO;
            photo = nil;
        }
        bPScrollView.photo = photo;
        ...
        if ([self.delegate respondsToSelector:@selector(photoBrowser:photoDidSelectView:atIndexPath:)]) {
            ...
            bPScrollView.callback = ^(id obj){
                [weakSelf.delegate photoBrowser:weakSelf photoDidSelectView:weakScrollBoxView atIndexPath:indexPath];
            };
            
        }
        
        [scrollBoxView addSubview:bPScrollView];
        bPScrollView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    }
...
- (void) delete{
    ...
    UIAlertView *removeAlert = [[UIAlertView alloc]
                                ...
                                message:nil
                                delegate:self
                                cancelButtonTitle:NSLocalizedStringForKey(@"取消")
                                otherButtonTitles:NSLocalizedStringForKey(@"确定"), nil];
    [removeAlert show];
}
...
- (void)showToView{
    MLPhotoBrowserPhoto *photo = self.photos[_currentIndexPath.item];
    MLPhotoBrowserPhotoScrollView* bPScrollView = [_collectionView viewWithTag:ScrollViewTAG];
    bPScrollView.photo = photo;
