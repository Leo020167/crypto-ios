...
- (void)loadImage {
    ...
    UIImage *cacheImage = [(SDImageCache *)([SDWebImageManager sharedManager].imageCache) imageFromCacheForKey:self.url.absoluteString];
...
- (void)didTap:(UITapGestureRecognizer *)tap {
    ...
    if (self.model.imageFromURLString) {
        [self removeGestureRecognizer:tap]; /** 单击后触发动画然后消失，在动画过程中避免再次触发单击事件引起冲突*/
