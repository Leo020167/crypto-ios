...
{
    if ([_delegate respondsToSelector:@selector(mediaFocusManager:editBtnClicked:editImage:)]) {
        [_delegate mediaFocusManager:self editBtnClicked:sender editImage:_focusViewController.mainImageView.image];
    }
...
- (ASMediaFocusController *)focusViewControllerForView:(UIView *)mediaView
{
    ...
    if ([_delegate respondsToSelector:@selector(mediaFocusManager:imageForView:)]) {
    ...
    viewController = [[[ASMediaFocusController alloc] initWithNibName:nil bundle:nil]autorelease];
    ...
    viewController.mainImageView.layer.borderWidth = 0.0;
    [viewController displayButton:_bDisplayButton];
    if (_bDownload) {
        viewController.mainImageView.alpha=0.5;
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            UIImage *image = nil;
            NSString *path = @"";
            
            if ([_delegate respondsToSelector:@selector(mediaFocusManager:mediaPathForView:)]) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [UIView animateWithDuration:0.2
                                 animations:^{
                                     viewController.mainImageView.alpha=1;
...
- (void)handleFocusGesture:(UIGestureRecognizer *)gesture
{
    ...
    if(focusViewController == nil)
        return;
    focusViewController.editBtn.hidden = YES;
    
    
    self.focusViewController = focusViewController;
    [self.focusViewController.editBtn addTarget:self action:@selector(editBtnClicked:) forControlEvents:UIControlEventTouchUpInside];
    ...
    focusViewController.mainImageView.layer.borderWidth = 0.0;
    ...
    [UIView animateWithDuration:self.animationDuration
                     animations:^{
                         ...
                         focusViewController.progressView.hidden = YES;
                     ...
                     completion:^(BOOL finished) {
                         ...
                         if(self.elasticAnimation)
                         {
                             [UIView animateWithDuration:self.animationDuration*kAnimateElasticDurationRatio
                              ...
                              completion:^(BOOL finished) {
                                  if (_bDownload) {
                                      if ([_delegate respondsToSelector:@selector(mediaFocusManager:downloadFile:)]) {
                                          [self.delegate mediaFocusManager:self downloadFile:mediaView];
                                          //[self startAnimating];
                                      }
                                      _bDownload = NO;
                                  }else{
...
- (void)installZoomView
{
    ...
    [_focusViewController displayButton:_bDisplayButton];

    _focusViewController.mainImageView.layer.borderWidth = BorderWidth;
...
- (void)uninstallZoomView
{
    ...
    if ([_delegate respondsToSelector:@selector(mediaFocusManagerUninstall)]) {
        [self.delegate mediaFocusManagerUninstall];
    }
}
...
- (void)handleDefocusGesture:(UIGestureRecognizer *)gesture
{
    ...
    _focusViewController.editBtn.hidden = YES;
    
    _focusViewController.mainImageView.image = _lastImage;
    _focusViewController.mainImageView.layer.borderWidth = 0.0;
...
- (void)startAnimating{
    self.focusViewController.progressView .hidden = NO;
    [self.focusViewController.progressView startAnimating];
}

- (void)stopAnimating{
    self.focusViewController.progressView.hidden = YES;
    [self.focusViewController.progressView stopAnimating];
}
