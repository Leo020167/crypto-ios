...
//            NSLog(@"-----%-----i== %ld     ======%@",j * perAngle,i,NSStringFromCGPoint(cachePoint));
            ...
            if (i==0) {
                CGSize size = [self sizeOfStringWithMaxSize:CGSizeMake(100, 70) textFont:self.descTextFont.pointSize aimString:self.valueDescArray[j]];
                UILabel *cacheLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, size.width, 70)];
                cacheLabel.font = self.descTextFont;
                cacheLabel.center = CGPointMake(self.chartOrigin.x + (self.chartRadius + size.width / 2 + 5) * sin(j * perAngle) ,self.chartOrigin.y - (self.chartRadius + size.height / 2 + 5) * cos(j * perAngle));
