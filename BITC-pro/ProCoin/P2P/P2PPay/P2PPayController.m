...
- (void)viewDidLoad {
    ...
    self.authShopLabel.text = NSLocalizedStringForKey(@"Stdelit 认证商家");
...
- (void)reqGetOrderDetailFinished:(id)result
{
    ...
    if([parser parseBaseIsOk:result]){
        ...
        _lbAmount.text = [NSString stringWithFormat:NSLocalizedStringForKey(@"您正在向Stdelit 认证卖家购买%@ USDT"), _model.amount];
