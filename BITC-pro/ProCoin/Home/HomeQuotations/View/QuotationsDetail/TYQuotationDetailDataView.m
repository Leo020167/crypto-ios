...
@interface TYQuotationDetailDataView()<KLineViewDelegate,KLineViewAndNetDataDelegate>
{
...
@implementation TYQuotationDetailDataView
...
- (void)initCoinQuotationDataView
{
    ...
    priceDecimalsDot = 0;
    buttonType = (CoinQuotationButtonType)[[[NSUserDefaults standardUserDefaults] objectForKey:CoinQuotationDetailButtonOptionsKey] integerValue];      //缓存值
...
- (void)reloadQuotationData:(CoinQuotationDataEntity *)dataEntity
{
    ...
    if([dataEntity.rate doubleValue]>= 0){
        _lastPriceLabel.textColor = QuotationGreenColor;
        _rateLabel.textColor = QuotationGreenColor;
        _lastPriceCNYLabel.textColor = QuotationGreenColor;
    }else{
        _lastPriceLabel.textColor = QuotationRedColor;
        _rateLabel.textColor = QuotationRedColor;
        _lastPriceCNYLabel.textColor = QuotationRedColor;
    }
...
- (IBAction)quotationTypeButtonPressed:(id)sender
{
    ...
    if(targetButton == _shareTimeButton){
        ...
        if(targetButton == _fifteenMinutesKLineButton){
            _dayKLineButton.selected = NO;
            _weekKLineButton.selected = NO;
            _oneHourKLineButton.selected = NO;
            _oneMinKLineButton.selected = NO;
    ...
    [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithInteger:buttonType] forKey:CoinQuotationDetailButtonOptionsKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
    _ma5Label.text = @"---";
    _ma10Label.text = @"---";
    _ma20Label.text = @"---";
    if([_delegate respondsToSelector:@selector(coinQuotationDataViewQuotationTypeDidChanged: dataType:)]){
        [_delegate coinQuotationDataViewQuotationTypeDidChanged:buttonType dataType:targetButton == _shareTimeButton ? 1 : 2];
    }
}
...
- (void)kLineViewAddData:(CoinQuotationDataEntity *)currentData
{
    ...
    }else if(_oneMinKLineButton.isSelected){
        [_kLineView addKLineData:currentData kLineType:KLineData1Minute];
    }else{
