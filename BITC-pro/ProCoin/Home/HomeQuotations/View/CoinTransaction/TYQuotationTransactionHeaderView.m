...
@interface TYQuotationTransactionHeaderView()<UITableViewDelegate,UITableViewDataSource,TextFieldToolBarDelegate,UITextFieldDelegate,PCCoinTransactionMenuViewDelegate>
{
...
/// 可用手数
@property (retain, nonatomic) IBOutlet UILabel *availHandAmountLabel;

/// 保证金
@property (retain, nonatomic) IBOutlet UILabel *bondLabel;

/// 符号
@property (retain, nonatomic) IBOutlet UILabel *symbolLabel;

/// 划转按钮
@property (retain, nonatomic) IBOutlet UIButton *transferBtn;

@end

@implementation TYQuotationTransactionHeaderView
...
- (void)awakeFromNib
{
    ...
    self.buyButton.frame = CGRectMake(20, 0, (SCREEN_WIDTH - 180) / 2.0, 36);
    self.sellButton.frame = CGRectMake(20 + (SCREEN_WIDTH - 180) / 2.0, 0, (SCREEN_WIDTH - 180) / 2.0, 36);
    [self.buyButton setTitle:NSLocalizedStringForKey(@"买入") forState:0];
    [self.sellButton setTitle:NSLocalizedStringForKey(@"卖出") forState:0];
    self.handAmountTF.keyboardType = UIKeyboardTypeDecimalPad;
}
...
- (void)resetDefaultUIValue
{
    if([_buySell isEqualToString:PCQuotationTransactionBuyType]){         //购买
        ...
        [_operationButton setBackgroundImage:[CommonUtil createImageWithColor:QuotationGreenColor] forState:UIControlStateNormal];
        [_operationButton setTitle:NSLocalizedStringForKey(@"买入") forState:UIControlStateNormal];
        _inputPriceView.layer.borderColor = RGBA(0, 173, 136, 1.0).CGColor;
    }else{              //出售
        _buyButton.selected = NO;
        _sellButton.selected = YES;
        self.transferBtn.hidden = YES;
        [_operationButton setBackgroundImage:[CommonUtil createImageWithColor:QuotationRedColor] forState:UIControlStateNormal];
        [_operationButton setTitle:NSLocalizedStringForKey(@"卖出") forState:UIControlStateNormal];
...
- (void)reloadHeaderViewConfig:(PCTradeConfigModel *)configEntity
{
    ...
    if([[_configEntity multiNumList] count] > 0){
        self.symbolLabel.text = configEntity.symbol;
...
- (void)reloadAmountOptionsView
{
    ...
    CGFloat unitButtonWidth = (_amountOptionsView.frame.size.width) / self.configEntity.openRateList.count;
    
        for(int i = 0 ; i < self.configEntity.openRateList.count; i++){
            UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
            [button setFrame:CGRectMake((unitButtonWidth) * i, 0.0, unitButtonWidth, _amountOptionsView.frame.size.height)];
            ...
            [button setTitle:[NSString stringWithFormat:@"%@%%", [self.configEntity.openRateList objectAtIndex:i]] forState:UIControlStateNormal];
...
- (void)amountOptionsButonPressed:(UIButton *)sender
{
    ...
    if ([self.checkOutEntity.buySell isEqualToString:@"buy"]) {
        //_handAmountTF.text = [self.configEntity.handList objectAtIndex:tag];
        double value = self.checkOutEntity.maxHand.doubleValue * ([[self.configEntity.openRateList objectAtIndex:tag] doubleValue] / 100.0);
        _handAmountTF.text = [NSString stringWithFormat:@"%.8f", value];
    } else {
        double value = self.checkOutEntity.availableAmount.doubleValue * ([[self.configEntity.openRateList objectAtIndex:tag] doubleValue] / 100.0);
        _handAmountTF.text = [NSString stringWithFormat:@"%.8f", value];
    }
    //通知发生了变化
    [self viewCoreDataDidChanged];
...
- (void)reloadCheckOutData:(PCTradeCheckOutModel *)checkOutEntity
{
    ...
    if ([checkOutEntity.buySell isEqualToString:@"buy"]) {
        self.availHandAmountLabel.text = [NSString stringWithFormat:@"%@%@%@", NSLocalizedStringForKey(@"可买"), checkOutEntity.maxHand, self.configEntity.symbol];
        self.bondLabel.text = [NSString stringWithFormat:@"%@ USDT", checkOutEntity.needMoney];
    }else{
        self.availHandAmountLabel.text = [NSString stringWithFormat:@"%@%@%@", NSLocalizedStringForKey(@"可用"), checkOutEntity.availableAmount, self.configEntity.symbol];
        self.bondLabel.text = [NSString stringWithFormat:@"%@ USDT", checkOutEntity.usdtAmount];
    }
}
