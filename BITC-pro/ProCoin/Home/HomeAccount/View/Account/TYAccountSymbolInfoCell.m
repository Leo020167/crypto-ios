...
@interface TYAccountSymbolInfoCell ()
...
@implementation TYAccountSymbolInfoCell
...
- (void)setModel:(PCAccountSymbolModel *)model{
    self.nameLabel.text = model.symbol;
    self.countLabel.text = [NSString stringWithFormat:@"%@", model.holdAmount];
...
- (UILabel *)freezeTitleLabel{
    if (!_freezeTitleLabel) {
        ...
        _freezeTitleLabel.text = NSLocalizedStringForKey(@"冻结");
