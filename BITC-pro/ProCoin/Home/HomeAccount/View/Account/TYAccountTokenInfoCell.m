...
- (void)setTokenModel:(PCAccountModel *)tokenModel{
    ...
    self.totalAssetTitleLabel.text = NSLocalizedStringForKey(@"总资产(AiCoin)");
    self.totalAssetLabel.text = tokenModel.assets;
    
    self.availableTitleLabel.text = NSLocalizedStringForKey(@"可用(AiCoin)");
    self.availableLabel.text = tokenModel.holdAmount;
    
    self.freezeTitleLabel.text = NSLocalizedStringForKey(@"冻结(AiCoin)");
...
- (UIButton *)getBtn{
    if (!_getBtn) {
        ...
        [_getBtn setTitle:NSLocalizedStringForKey(@"查看如何获取AiCoin？") forState:0];
