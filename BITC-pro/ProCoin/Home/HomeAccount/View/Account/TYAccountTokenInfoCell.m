...
- (void)setTokenModel:(PCAccountModel *)tokenModel{
    ...
    self.totalAssetTitleLabel.text = NSLocalizedStringForKey(@"总资产(TFU)");
    self.totalAssetLabel.text = tokenModel.assets;
    
    self.availableTitleLabel.text = NSLocalizedStringForKey(@"可用(TFU)");
    self.availableLabel.text = tokenModel.holdAmount;
    
    self.freezeTitleLabel.text = NSLocalizedStringForKey(@"冻结(TFU)");
...
- (UIButton *)getBtn{
    if (!_getBtn) {
        ...
        [_getBtn setTitle:NSLocalizedStringForKey(@"查看如何获取TFU？") forState:0];
