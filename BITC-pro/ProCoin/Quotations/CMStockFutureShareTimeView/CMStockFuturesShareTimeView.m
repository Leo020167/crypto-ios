...
@interface CMStockFuturesShareTimeView()
{
    ...
    NSInteger totalMinutes;         //总分钟数,241根数据
...
@implementation CMStockFuturesShareTimeView
...
{
    midPrice = [yesterdayClose doubleValue];
    //解析数据，
    if (TTIsStringWithAnyText(historyData)) {
        ...
        [shareTimeData addObjectsFromArray:[[dataArr reverseObjectEnumerator] allObjects]];
        [self setNeedsDisplay];
    }
}
...
- (void)addOneShareTimeViewData:(CoinQuotationDataEntity *)dataEntity
{
    ...
    if(checkIsStringWithAnyText(std.time)){
        CMShareTimeBaseData *lastEntity = [shareTimeData lastObject];
        if([std.time isEqualToString:lastEntity.time]){
            [shareTimeData replaceObjectAtIndex:[shareTimeData count] - 1 withObject:std];
        }else{
            [shareTimeData addObject:std];
        }
    }
    [self setNeedsDisplay];
}
...
- (void)drawBackgroundLineText:(CGContextRef)context
{
    ...
    for(int i = 0; i <= _totalColumn;i++){
        NSString *dateStr = [dateArr objectAtIndex:i];
        CGSize stringSize = [CommonUtil getPerfectSizeByText:dateStr andFontSize:10.0f andWidth:1000];
        if(i == 0 ){
            [dateStr drawAtPoint:CGPointMake(0.0, self.frame.size.height - 15) withAttributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:textColor}];
        }else if(i == _totalColumn){
            [dateStr drawAtPoint:CGPointMake(self.frame.size.width - stringSize.width, self.frame.size.height - 15) withAttributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:textColor}];
        }else{
            [dateStr drawAtPoint:CGPointMake((i * _girdWidth) - (stringSize.width / 2.0f), self.frame.size.height - 15) withAttributes:@{NSFontAttributeName:font,NSForegroundColorAttributeName:textColor}];
        }
    }
...
- (void)drawShareTimePoint:(CGContextRef)context
{
    ...
    for(NSInteger i = 0; i < [shareTimeData count]; i++){
        ...
        if(i == [shareTimeData count] - 1){
            if(i >= totalMinutes - 1){      //不再需要显示呼吸点
                [self stopArcViewAnimation];
            }else{      //最后一根数据显示呼吸点
                [self changeArcViewCenter:CGPointMake(perMinuterWidth * i, pointY)];
            }
        }
    ...
    [dataPointArr insertObject:[NSValue valueWithCGPoint:CGPointMake(0.0, topBlankHeight + mainViewHeight / 2.0f)] atIndex:0];
    [dataPointArr addObject:[NSValue valueWithCGPoint:CGPointMake((CGFloat)([shareTimeData count] - 1) * perMinuterWidth, topBlankHeight + mainViewHeight / 2.0f)]];
