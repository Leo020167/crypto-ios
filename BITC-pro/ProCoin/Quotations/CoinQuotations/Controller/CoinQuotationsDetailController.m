...
#import "CoinQuotationsDetailController.h"
...
#import "CoinQuotationSegmentDealController.h"
#import "PCQuotationDealModel.h"
#import "NetWorkManage+Quotation.h"


@interface CoinQuotationsDetailController ()<CoinQuotationDataViewDelegate,YNPageViewControllerDelegate,YNPageViewControllerDataSource>
{
    ...
    CoinQuotationsSegmentGearController *gearVC;
    CoinQuotationsSegmentInfoController *infoVC;
    CoinQuotationSegmentDealController *dealVC;
...
@property (retain, nonatomic) CoinIntroEntity *introEntity;                 //币种介绍对象

@property (retain, nonatomic) CoinQuotationDataView *quotationView;             //行情页面
...
@implementation CoinQuotationsDetailController
...
- (void)viewDidLoad
{
    ...
    [_buyButton setBackgroundImage:[CommonUtil createImageWithColor:[TradeUtil textColorWithQuotationNumber:1.0]] forState:UIControlStateNormal];
    [_sellButton setBackgroundImage:[CommonUtil createImageWithColor:[TradeUtil textColorWithQuotationNumber:-1.0]] forState:UIControlStateNormal];
...
#pragma mark - 懒加载
- (CoinQuotationDataView *)quotationView
{
    if(!_quotationView){
        _quotationView = (CoinQuotationDataView *)[[[[NSBundle mainBundle] loadNibNamed:@"CoinQuotationDataView" owner:nil options:nil] lastObject] retain];
...
{
    if(!_segmentArrayVC){
        dealVC = [[CoinQuotationSegmentDealController alloc] init];
        infoVC = [[CoinQuotationsSegmentInfoController alloc] init];
        _segmentArrayVC = [[NSArray arrayWithObjects:dealVC,infoVC,nil] retain];
    }
...
- (NSArray *)segmentArrayTitle
{
    if(!_segmentArrayTitle){
        _segmentArrayTitle = [[NSArray arrayWithObjects:NSLocalizedStringForKey(@"成交"),NSLocalizedStringForKey(@"简介"),nil] retain];
    }
...
- (void)reqQuotationKLineData:(NSString *)timestamp
{
    ...
    if(checkIsStringWithAnyText(kLineType)){
        [[NetWorkManage shareSingleNetWork] reqQuotationKLineData:self symbol:_symbol timestamp:timestamp pageSize:@"" kLineType:kLineType type:@"h" finishedCallback:@selector(reqQuotationKLineDataFinished:) failedCallback:@selector(reqQuotationKLineDataFailed:)];
    }
...
- (void)reqQuotationKLineDataFinished:(NSDictionary *)json
{
    ...
    if([self checkJsonIsSuccess:json]){
        ...
        if(quotationButtonType == CoinQuotationButtonType_ShareTime){
            currentKLineType = PCQuotationKLineTypeShareTime;
        }else if(quotationButtonType == CoinQuotationButtonType_FifteenKLine){
            currentKLineType = PCQuotationKLineTypeMin15;
        }else if(quotationButtonType == CoinQuotationButtonType_OneHourKLine){
            currentKLineType = PCQuotationKLineTypeHour1;
...
- (void)reqQuotationKLineDataFailed:(NSDictionary *)json{
...
#pragma mark - 请求数据
/** 请求币种简介*/
- (void)reqCoinInfoData
{
    [[NetWorkManage shareSingleNetWork] reqCoinInfo:self symbol:_symbol type:@"" finishedCallback:@selector(reqCoinDataFinished:) failedCallback:nil];
}

- (void)reqCoinDataFinished:(NSDictionary *)json
{
    if([self checkJsonIsSuccess:json]){
        NSDictionary *dataDic = [json objectForKey:@"data"];
        NSDictionary *coinDic = [dataDic objectForKey:@"coin"];
        self.introEntity = [[[CoinIntroEntity alloc] initWithJson:coinDic] autorelease];
        [infoVC reloadCoinQuotationSegmentInfo:self.introEntity];
    }else{
...
- (UIScrollView *)pageViewController:(YNPageViewController *)pageViewController pageForIndex:(NSInteger)index
{
    ...
    if([baseVC isKindOfClass:[CoinQuotationsSegmentGearController class]]){
        CoinQuotationsSegmentGearController *vc = baseVC;
        return [vc gearTableView];
    }else if([baseVC isKindOfClass:[CoinQuotationSegmentDealController class]]){
        CoinQuotationSegmentDealController *vc = baseVC;
        return [vc dealTableView];
    }else if([baseVC isKindOfClass:[CoinQuotationsSegmentInfoController class]]){
        CoinQuotationsSegmentInfoController *vc = baseVC;
        return [vc infoTableView];
    }
    return nil;
}
