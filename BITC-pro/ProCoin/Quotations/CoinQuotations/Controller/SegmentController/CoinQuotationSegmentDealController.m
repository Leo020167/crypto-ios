...
@interface CoinQuotationSegmentDealController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation CoinQuotationSegmentDealController
...
- (UIView *)dealHeaderView
{
    if(_dealHeaderView == nil){
        self.dealHeaderView = [[[NSBundle mainBundle] loadNibNamed:@"QuotationsTradeDealHeaderView" owner:nil options:nil] lastObject];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    buySellLabel.text = [entity.buySell isEqualToString:PCQuotationTransactionBuyType] ? NSLocalizedStringForKey(@"看涨(做多)"):NSLocalizedStringForKey(@"看跌(做空)");
    buySellLabel.textColor = [entity.buySell isEqualToString:PCQuotationTransactionBuyType] ? [TradeUtil textColorWithQuotationNumber:1.0]:[TradeUtil textColorWithQuotationNumber:-1.0];
