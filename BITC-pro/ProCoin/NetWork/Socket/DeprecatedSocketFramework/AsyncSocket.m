...
- (NSUInteger)readLengthForTerm {
	...
	if (maxLength > 0) return MIN((CFIndex)result, ((CFIndex)maxLength - bytesDone));
	else return result;
}
...
- (CFSocketRef)getCFSocket {
	if (theSocket4) return theSocket4;
	else return theSocket6;
}
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket {
	if (newNativeSocket) {
		...
		if (![newSocket createStreamsFromNative:newNativeSocket error:nil]) goto Failed;

		if (parentSocket == theSocket4) newSocket->theNativeSocket4 = newNativeSocket;
		else newSocket->theNativeSocket6 = newNativeSocket;

		if ([theDelegate respondsToSelector:@selector(onSocket:didAcceptNewSocket:)]) [theDelegate onSocket:self didAcceptNewSocket:newSocket];
...
- (void)doSocketOpen:(CFSocketRef)sock withCFSocketError:(CFSocketError)socketError {
	...
	if (sock == theSocket4) theNativeSocket4 = nativeSocket;
	else theNativeSocket6 = nativeSocket;
...
- (NSString *)connectedHost {
	if (theSocket4) return [self connectedHostFromCFSocket4:theSocket4];

	if (theSocket6) return [self connectedHostFromCFSocket6:theSocket6];

	if (theNativeSocket4 > 0) return [self connectedHostFromNativeSocket4:theNativeSocket4];

	if (theNativeSocket6 > 0) return [self connectedHostFromNativeSocket6:theNativeSocket6];

	return nil;
}

- (UInt16)connectedPort {
	if (theSocket4) return [self connectedPortFromCFSocket4:theSocket4];

	if (theSocket6) return [self connectedPortFromCFSocket6:theSocket6];

	if (theNativeSocket4 > 0) return [self connectedPortFromNativeSocket4:theNativeSocket4];

	if (theNativeSocket6 > 0) return [self connectedPortFromNativeSocket6:theNativeSocket6];

	return 0;
}

- (NSString *)localHost {
	if (theSocket4) return [self localHostFromCFSocket4:theSocket4];

	if (theSocket6) return [self localHostFromCFSocket6:theSocket6];

	if (theNativeSocket4 > 0) return [self localHostFromNativeSocket4:theNativeSocket4];

	if (theNativeSocket6 > 0) return [self localHostFromNativeSocket6:theNativeSocket6];

	return nil;
}

- (UInt16)localPort {
	if (theSocket4) return [self localPortFromCFSocket4:theSocket4];

	if (theSocket6) return [self localPortFromCFSocket6:theSocket6];

	if (theNativeSocket4 > 0) return [self localPortFromNativeSocket4:theNativeSocket4];

	if (theNativeSocket6 > 0) return [self localPortFromNativeSocket6:theNativeSocket6];
...
- (NSData *)connectedAddress {
	...
	if (theSocket4) theSocket = theSocket4;
	else theSocket = theSocket6;
...
- (BOOL)areStreamsConnected {
	...
	if (theReadStream != NULL) {
		...
		if (!((s == kCFStreamStatusOpen) || (s == kCFStreamStatusReading) || (s == kCFStreamStatusError))) return NO;
	} else return NO;
...
- (NSString *)description {
	...
	if (theFlags & kDisconnectAfterReads) {
		if (theFlags & kDisconnectAfterWrites) [ms appendString:@", will disconnect after reads & writes"];
		else [ms appendString:@", will disconnect after reads"];
	} else if (theFlags & kDisconnectAfterWrites) {
...
- (void)doCFSocketCallback	:(CFSocketCallBackType)type
	...
	switch (type) {
		case kCFSocketConnectCallBack :
			...
			if (pData) [self doSocketOpen:sock withCFSocketError:kCFSocketError];
			else [self doSocketOpen:sock withCFSocketError:kCFSocketSuccess];
			break;
