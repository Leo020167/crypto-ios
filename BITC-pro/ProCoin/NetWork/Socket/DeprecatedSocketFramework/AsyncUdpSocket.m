...
- (NSString *)localHost {
	...
	if (theSocket4) return [self localHost:theSocket4];
	else return [self localHost:theSocket6];
}

- (UInt16)localPort {
	if (cachedLocalPort > 0) return cachedLocalPort;

	if (theSocket4) return [self localPort:theSocket4];
	else return [self localPort:theSocket6];
}

- (NSString *)connectedHost {
	if (cachedConnectedHost) return cachedConnectedHost;

	if (theSocket4) return [self connectedHost:theSocket4];
	else return [self connectedHost:theSocket6];
}

- (UInt16)connectedPort {
	if (cachedConnectedPort > 0) return cachedConnectedPort;

	if (theSocket4) return [self connectedPort:theSocket4];
	else return [self connectedPort:theSocket6];
}
...
- (unsigned int)maximumTransmissionUnit {
	...
	if (theSocket4) theNativeSocket = CFSocketGetNative(theSocket4);
	else if (theSocket6) theNativeSocket = CFSocketGetNative(theSocket6);
	else return 0;
...
- (BOOL)sendData:(NSData *)data toHost:(NSString *)host port:(UInt16)port withTimeout:(NSTimeInterval)timeout tag:(long)tag {
	...
	if (address4 && theSocket4) packet = [[AsyncSendPacket alloc] initWithData:data address:address4 timeout:timeout tag:tag];
	else if (address6 && theSocket6) packet = [[AsyncSendPacket alloc] initWithData:data address:address6 timeout:timeout tag:tag];
	else return NO;
...
- (BOOL)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag {
	...
	if (([remoteAddr length] == sizeof(struct sockaddr_in)) && !theSocket4) return NO;

	if (([remoteAddr length] == sizeof(struct sockaddr_in6)) && !theSocket6) return NO;
...
- (CFSocketRef)socketForPacket:(AsyncSendPacket *)packet {
	if (!theSocket4) return theSocket6;

	if (!theSocket6) return theSocket4;
...
- (void)doSend:(CFSocketRef)theSocket {
	if (theCurrentSend != nil) {
		...
		if ([self canAcceptBytes:theSocket]) {
			...
			if (theSocket == theSocket4) theFlags &= ~kSock4CanAcceptBytes;
			else theFlags &= ~kSock6CanAcceptBytes;
...
- (void)doCFSocketCallback	:(CFSocketCallBackType)type
	...
	switch (type) {
		case kCFSocketReadCallBack:
			...
			if (sock == theSocket4) theFlags |= kSock4HasBytesAvailable;
			else theFlags |= kSock6HasBytesAvailable;
			[self doReceive:sock];
			break;

		case kCFSocketWriteCallBack:

			if (sock == theSocket4) theFlags |= kSock4CanAcceptBytes;
			else theFlags |= kSock6CanAcceptBytes;
			[self doSend:sock];
			break;
