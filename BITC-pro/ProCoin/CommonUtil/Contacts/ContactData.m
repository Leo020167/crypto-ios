...
+ (NSArray *)contactsArray {
	...
	if ([ABContact getAddressBook]) {
		thePeople = (NSArray *)ABAddressBookCopyArrayOfAllPeople([ABContact getAddressBook]);
...
+ (ABContact *)byPhoneNumberAndLabelToGetContact:(NSString *)phone withLabel:(NSString *)label {
	...
	for (ABContact *contast in array) {
		...
		for (NSDictionary *dic in phoneArray) {
			...
			if ([aPhone isEqualToString:phone] && [aLabel isEqualToString:label]) return (ABContact *)contast;
		}
	}
...
+ (ABContact *)byPhoneNumberAndNameToGetContact:(NSString *)name withPhone:(NSString *)phone {
	...
	for (ABContact *contast in array) {
		...
		for (NSDictionary *dic in phoneArray) {
			...
			//	NSString *aLabel = [ContactData getPhoneLabelFromDic:dic];
			if ([aPhone isEqualToString:phone] && [name isEqualToString:contast.contactName]) return (ABContact *)contast;
		}
	}
...
+ (ABContact *)byNameToGetContact:(NSString *)name {
	...
	for (ABContact *contast in array) {
		if ([contast.contactName isEqualToString:name]) return (ABContact *)contast;
	}
...
+ (ABContact *)byPhoneNumberlToGetContact:(NSString *)phone withLabel:(NSString *)label {
	...
	for (ABContact *contast in array) {
		...
		for (NSDictionary *dic in phoneArray) {
			...
			// NSString *aLabel = [ContactData getPhoneLabelFromDic:dic];
			if ([aPhone isEqualToString:phone] && [label isEqualToString:@"未知"]) return (ABContact *)contast;
		}
	}
...
+ (NSArray *)getPhoneNumberAndPhoneLabelArray:(ABContact *)contact {
	...
	for (i = 0; i < ABMultiValueGetCount(phoneMulti); i++) {
		...
		phoneDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:contact.contactName, KPHONENAMEDICDEFINE, phone, KPHONENUMBERDICDEFINE, label, KPHONELABELDICDEFINE, nil];
...
+ (NSArray *)getPhoneNumberAndPhoneLabelArrayFromABRecodID:(ABRecordRef)person withABMultiValueIdentifier:(ABMultiValueIdentifier)identifierForValue {
	...
	phoneDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:nameStr, KPHONENAMEDICDEFINE, phone, KPHONENUMBERDICDEFINE, label, KPHONELABELDICDEFINE, nil];
	[phoneArray addObject:phoneDic];
	CFRelease(phoneMulti);
	return phoneArray;
}
...
+ (NSString *)getPhoneLabelFromDic:(NSDictionary *)Phonedic {
	...
	if ([PhoneLabel isEqualToString:@"_$!<Mobile>!$_"]) PhoneLabel = @"移动电话";
	else if ([PhoneLabel isEqualToString:@"_$!<Home>!$_"]) PhoneLabel = @"住宅";
	else if ([PhoneLabel isEqualToString:@"_$!<Work>!$_"]) PhoneLabel = @"工作";
	else if ([PhoneLabel isEqualToString:@"_$!<Main>!$_"]) PhoneLabel = @"主要";
	else if ([PhoneLabel isEqualToString:@"_$!<HomeFAX>!$_"]) PhoneLabel = @"住宅传真";
	else if ([PhoneLabel isEqualToString:@"_$!<WorkFAX>!$_"]) PhoneLabel = @"工作传真";
	else if ([PhoneLabel isEqualToString:@"_$!<Pager>!$_"]) PhoneLabel = @"传呼";
	else if ([PhoneLabel isEqualToString:@"_$!<Other>!$_"]) PhoneLabel = @"其它";
	return PhoneLabel;
}
...
+ (BOOL)doesStringContain:(NSString *)string Withstr:(NSString *)charcter {
	...
	for (int i = 0; i < [string length]; i++) {
		...
		if ([chr isEqualToString:charcter]) return TRUE;
	}
...
+ (NSString *)equalContactByAddressBookContacts:(NSString *)name withPhone:(NSString *)phone withLabel:(NSString *)label PhoneOrLabel:(BOOL)isPhone withFavorite:(BOOL)isFavorite {
	...
	if (isFavorite) contact = [ContactData byNameToGetContact:name];

	if (!contact) contact = [ContactData byPhoneNumberAndLabelToGetContact:phone withLabel:label];

	if (!contact) contact = [ContactData byPhoneNumberAndNameToGetContact:name withPhone:phone];

	if ([label isEqualToString:@"未知"] && (contact == nil)) contact = [ContactData byPhoneNumberlToGetContact:phone withLabel:label];
...
+ (NSString *)getContactsNameByPhoneNumberAndLabel:(NSString *)phone withLabel:(NSString *)label {
	...
	for (ABContact *contast in array) {
		...
		for (NSDictionary *dic in phoneArray) {
			...
			if ([aPhone isEqualToString:phone] && [aLabel isEqualToString:label]) return contast.contactName;
		}
	}
...
+ (BOOL)removeSelfFromAddressBook:(ABContact *)contact withErrow:(NSError **)error {
	if (!ABAddressBookRemoveRecord([ABContact getAddressBook], contact.record, (CFErrorRef *)error)) return NO;

	return ABAddressBookSave([ABContact getAddressBook], (CFErrorRef *)error);
}
