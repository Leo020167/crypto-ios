...
{
    if (addressBook==nil) {
            addressBook= [ABContact phoneAddressBookCreate];
    }
    return addressBook;
}


+(ABAddressBookRef)phoneAddressBookCreate
{
    @try {
...
{
	ABRecordRef contactrec = ABAddressBookGetPersonWithRecordID([ABContact getAddressBook], recordID);
...
- (NSArray *) dictionaryArrayForProperty: (ABPropertyID) aProperty
{
	...
	NSInteger num = MIN(valueArray.count, labelArray.count);
