...
//  BProject
//
//  Created by taojinroad on 14-6-18.
...
- (BOOL)imageExistsFromBundle:(NSString*)URL {
    NSString* path = [CommonUtil TTPathForBundleResource:[URL substringFromIndex:9]];
    NSFileManager* fm = [NSFileManager defaultManager];
...
- (BOOL)imageExistsFromDocuments:(NSString*)URL {
    NSString* path = [CommonUtil TTPathForDocumentsResource:[URL substringFromIndex:12]];
    NSFileManager* fm = [NSFileManager defaultManager];
    return [fm fileExistsAtPath:path];
}
