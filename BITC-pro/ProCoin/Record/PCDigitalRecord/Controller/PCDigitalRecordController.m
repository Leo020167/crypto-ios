...
- (void)getQuerySumAction{
    [YYRequestUtility Post:@"/pro/order/querySum.do" addParameters:@{@"accountType" : self.accountType} progress:nil success:^(NSDictionary *responseDict) {
        if ([responseDict[@"code"] intValue] == 200) {
            ...
            [tokenText yy_setFont:UIFontMake(13) range:NSMakeRange(0, NSLocalizedStringForKey(@"获得FGEC").length)];
            [tokenText yy_setColor:UIColorMakeWithHex(@"#999999") range:NSMakeRange(0, NSLocalizedStringForKey(@"获得FGEC").length)];
